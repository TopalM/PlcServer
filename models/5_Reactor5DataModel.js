...
/** ---------- Factory ---------- **/
export default function makeReactor5DataModel(plcConn) {
  const reactor5DataSchema = new mongoose.Schema(
    {
  ...
  reactor5DataSchema.index({ DataTime: -1 });

  Object.entries(PRECISION).forEach(([path, places]) => {
    const limits = LIMITS[path] || {};
    if (reactor5DataSchema.path(path)) {
      reactor5DataSchema.path(path).set(sanitizeRound(places, limits.min, limits.max));
    }
  });

  reactor5DataSchema.pre("save", async function (next) {
    if (!this.isNew) return next();

    const last = await this.constructor.findOne({}, { _id: 0 }).sort({ DataTime: -1 }).lean();

    if (last) {
      const paths = Object.keys(reactor5DataSchema.paths).filter((p) => p !== "_id" && p !== "DataTime");
  ...
  return plcConn.models.Reactor5Data || plcConn.model("Reactor5Data", reactor5DataSchema, "reactor5Data");
}
