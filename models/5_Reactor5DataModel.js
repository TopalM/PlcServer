...
export default function makeReactor5DataModel(plcConn) {
  const reactor5DataSchema = new mongoose.Schema(
    ...
    { collection: "reactor5Data", timestamps: false, versionKey: false, strict: true, minimize: true }
  );
  ...
  Object.entries(PRECISION).forEach(([p, places]) => {
    const lim = LIMITS[p] || {};
    if (reactor5DataSchema.path(p)) {
      reactor5DataSchema.path(p).set(sanitizeRound(places, lim.min, lim.max));
    }
  });
