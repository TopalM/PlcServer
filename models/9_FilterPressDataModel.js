...
const LIMITS = {
  DropTank2Agirlik: { min: -50000, max: 50000 },
  DropTank4Agirlik: { min: -50000, max: 50000 },
  DropTank6Agirlik: { min: -50000, max: 50000 },
  DropTank8Agirlik: { min: -50000, max: 50000 },
  DropTank10Agirlik: { min: -50000, max: 50000 },
...
export default function makeFilterPressDataModel(plcConn) {
  const filterPressSchema = new mongoose.Schema(
    {
      DataTime: { type: Date, required: true, index: true, unique: true },
    ...
    { collection: "filterPressData", timestamps: false, versionKey: false, strict: true, minimize: true }
  );
  ...
  Object.entries(PRECISION).forEach(([p, places]) => {
    const lim = LIMITS[p] || {};
    if (filterPressSchema.path(p)) {
      filterPressSchema.path(p).set(sanitizeRound(places, lim.min, lim.max));
    }
  });
