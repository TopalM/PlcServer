...
export default function makeReactor6DataModel(plcConn) {
  const reactor6DataSchema = new mongoose.Schema(
    ...
    { collection: "reactor6Data", timestamps: false, versionKey: false, strict: true, minimize: true }
  );
  ...
  Object.entries(PRECISION).forEach(([p, places]) => {
    const lim = LIMITS[p] || {};
    if (reactor6DataSchema.path(p)) {
      reactor6DataSchema.path(p).set(sanitizeRound(places, lim.min, lim.max));
    }
  });
