...
const LIMITS = {
  ...
  AlcoholFlowmeter: { min: -20_000, max: 20000 },
};

/** ---------- Factory ---------- **/
export default function makeReactor6DataModel(plcConn) {
  const reactor6DataSchema = new mongoose.Schema(
    {
  ...
  reactor6DataSchema.index({ DataTime: -1 });

  Object.entries(PRECISION).forEach(([path, places]) => {
    const limits = LIMITS[path] || {};
    if (reactor6DataSchema.path(path)) {
      reactor6DataSchema.path(path).set(sanitizeRound(places, limits.min, limits.max));
    }
  });

  reactor6DataSchema.pre("save", async function (next) {
    if (!this.isNew) return next();

    const last = await this.constructor.findOne({}, { _id: 0 }).sort({ DataTime: -1 }).lean();

    if (last) {
      const paths = Object.keys(reactor6DataSchema.paths).filter((p) => p !== "_id" && p !== "DataTime");
  ...
  return plcConn.models.Reactor6Data || plcConn.model("Reactor6Data", reactor6DataSchema, "reactor6Data");
}
