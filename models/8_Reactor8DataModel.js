...
export default function makeReactor8DataModel(plcConn) {
  ...
  reactor8DataSchema.index({ DataTime: -1 });

  Object.entries(PRECISION).forEach(([path, places]) => {
    const limits = LIMITS[path] || {};
    if (reactor8DataSchema.path(path)) {
      reactor8DataSchema.path(path).set(sanitizeRound(places, limits.min, limits.max));
    }
  });
