...
export default function makeReactor8DataModel(plcConn) {
  const reactor8DataSchema = new mongoose.Schema(
    {
    ...
    { collection: "reactor8Data", timestamps: false, versionKey: false, strict: true, minimize: true }
  );
  ...
  Object.entries(PRECISION).forEach(([p, places]) => {
    const lim = LIMITS[p] || {};
    if (reactor8DataSchema.path(p)) {
      reactor8DataSchema.path(p).set(sanitizeRound(places, lim.min, lim.max));
    }
  });

  reactor8DataSchema.pre("save", async function (next) {
    if (!this.isNew) return next();
    const last = await this.constructor.findOne({}, { _id: 0 }).sort({ DataTime: -1 }).lean();
    if (last) {
      const paths = Object.keys(reactor8DataSchema.paths).filter((p) => p !== "_id" && p !== "DataTime");
  ...
  return plcConn.models.Reactor8Data || plcConn.model("Reactor8Data", reactor8DataSchema, "reactor8Data");
}
