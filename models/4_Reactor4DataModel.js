...
export default function makeReactor4DataModel(plcConn) {
  const reactor4DataSchema = new mongoose.Schema(
    ...
    { collection: "reactor4Data", timestamps: false, versionKey: false, strict: true, minimize: true }
  );
  ...
  Object.entries(PRECISION).forEach(([p, places]) => {
    const lim = LIMITS[p] || {};
    if (reactor4DataSchema.path(p)) {
      reactor4DataSchema.path(p).set(sanitizeRound(places, lim.min, lim.max));
    }
  });
