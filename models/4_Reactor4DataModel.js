...
/** ---------- Factory ---------- **/
export default function makeReactor4DataModel(plcConn) {
  const reactor4DataSchema = new mongoose.Schema(
    {
  ...
  reactor4DataSchema.index({ DataTime: -1 });

  Object.entries(PRECISION).forEach(([path, places]) => {
    const limits = LIMITS[path] || {};
    if (reactor4DataSchema.path(path)) {
      reactor4DataSchema.path(path).set(sanitizeRound(places, limits.min, limits.max));
    }
  });

  reactor4DataSchema.pre("save", async function (next) {
    if (!this.isNew) return next();

    const last = await this.constructor.findOne({}, { _id: 0 }).sort({ DataTime: -1 }).lean();

    if (last) {
      const paths = Object.keys(reactor4DataSchema.paths).filter((p) => p !== "_id" && p !== "DataTime");
  ...
  return plcConn.models.Reactor4Data || plcConn.model("Reactor4Data", reactor4DataSchema, "reactor4Data");
}
