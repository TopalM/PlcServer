...
export default function makeReactor7DataModel(plcConn) {
  const reactor7DataSchema = new mongoose.Schema(
    {
    ...
    { collection: "reactor7Data", timestamps: false, versionKey: false, strict: true, minimize: true }
  );
  ...
  Object.entries(PRECISION).forEach(([p, places]) => {
    const lim = LIMITS[p] || {};
    if (reactor7DataSchema.path(p)) {
      reactor7DataSchema.path(p).set(sanitizeRound(places, lim.min, lim.max));
    }
  });

  reactor7DataSchema.pre("save", async function (next) {
    if (!this.isNew) return next();
    const last = await this.constructor.findOne({}, { _id: 0 }).sort({ DataTime: -1 }).lean();
    if (last) {
      const paths = Object.keys(reactor7DataSchema.paths).filter((p) => p !== "_id" && p !== "DataTime");
  ...
  return plcConn.models.Reactor7Data || plcConn.model("Reactor7Data", reactor7DataSchema, "reactor7Data");
}
