...
export default function makeReactor7DataModel(plcConn) {
  ...
  reactor7DataSchema.index({ DataTime: -1 });

  Object.entries(PRECISION).forEach(([path, places]) => {
    const limits = LIMITS[path] || {};
    if (reactor7DataSchema.path(path)) {
      reactor7DataSchema.path(path).set(sanitizeRound(places, limits.min, limits.max));
    }
  });
